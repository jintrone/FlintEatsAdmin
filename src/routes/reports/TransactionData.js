...
export default class TransactionData extends React.Component {
  constructor() {
    super();
  ...
  renderReport() {
    ...
    return (
      <div>
        <h2 style={{textAlign: 'center', width: '100%'}}>Double Up Food Bucks Transaction Data</h2>
        ...
        <Row>
          <Col sm={6} style={{padding: 25, height: 425}}>
            <h4 className='text-center'>Daily Earn and Spend</h4>
        ...
        <Row>
          <h4 style={{textAlign: 'center', width: '100%'}}>Transactions Stats vs. Previous Year</h4>
          <Col sm={4}>
            <PanelContainer controls={false}>
              <Panel ref={(c) => this.txn = c} >
                <PanelBody style={{padding: 25}}>
                  <h4 className='text-center'>Number of Transactions<br/>(% Change vs. Previous Year)</h4>
          ...
          <Col sm={4}>
            <PanelContainer controls={false}>
              <Panel ref={(c) => this.txnEarn = c} >
                <PanelBody style={{padding: 25}}>
                  <h4 className='text-center'>Total Amount Earned <br/>(% Change vs. Previous Year)</h4>
                  <div className='text-center' style={{height: 30, fontSize: 32}}>
                    <Icon glyph={this.state.transactions != null
          ...
          <Col sm={4}>
            <PanelContainer controls={false}>
              <Panel ref={(c) => this.txnSpend = c} >
                <PanelBody style={{padding: 25}}>
                  <h4 className='text-center'>Total Amount Spent <br/>(% Change vs. Previous Year)</h4>
                  <div className='text-center' style={{height: 30, fontSize: 32}}>
                    <Icon glyph={this.state.transactions != null
                                  && (this.state.transactions.spend/this.state.transactions.spendLY) >= 1
                                      ? 'icon-fontello-up-dir-1'
                                      : 'icon-fontello-down-dir-1'} />
                    {this.state.transactions.spend.replace(/\B(?=(\d{3})+(?!\d))/g, ",")
                      + ' ('
                      + (isFinite(((this.state.transactions.spend/this.state.transactions.spendLY)*100)-100)
                        ? (((this.state.transactions.spend/this.state.transactions.spendLY)*100)-100).toFixed(2)
                        : '--')
                      + '%)'}
                    </div>
                </PanelBody>
        ...
        <Row>
          <Col sm={12}>
            <h4 style={{textAlign: 'center', width: '100%'}}>Transactions by Location</h4>
            <br/>
            <Table ref={(c) => this.marketTable = c} className='display' cellSpacing='0' width='100%'>
              <thead>
                <tr>
                  <td>Location</td>
                  <td>Earn Transactions</td>
                  <td>Amount Earned ($)</td>
        ...
        <Row>
          <Col sm={12}>
            <h4 style={{textAlign: 'center', width: '100%'}}>Transactions by Date</h4>
            <br/>
      ...
      table
        ...
        .dataTable({
          ...
          footerCallback: function(row, data, start, end, display) {
            ...
            var earnCount = api.column(-4).data().reduce(function(a, b) {
                                                            return parseInt(a) + parseInt(b);
                                                          }, 0);
            var earnTotal = api.column(-3).data().reduce(function(a, b) {
                                                            return parseFloat(a) + parseFloat(b);
                                                          }, 0);
            var spendCount = api.column(-2).data().reduce(function(a, b) {
                                                            return parseInt(a) + parseInt(b);
                                                          }, 0);
            var spendTotal = api.column(-1).data().reduce(function(a, b) {
                                                            return parseFloat(a) + parseFloat(b);
                                                          }, 0);
            $(api.column(-4).footer()).html(earnCount);
            $(api.column(-3).footer()).html(earnTotal.toFixed(2));
            $(api.column(-2).footer()).html(spendCount);
            $(api.column(-1).footer()).html(spendTotal.toFixed(2));
          }
      });
    }
  }
